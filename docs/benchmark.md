...
## B-001 — TED "황금 기준선" (Benchmarker 첫 과업) · status: accepted — 활용1·2·3 구현됨(2026-06-21)

> ✅ **채택**(2026-06-21). 활용 1(백분위 점수 레이어) **메트릭 완성**: WPM·filler/분·**휴지/분·슬라이드밀도** 4종 모두 점수화. `lib/analysis/baselines/{talk,pitch,lecture}.json` + `percentile.ts` + report API/뷰 + 장르 선택 UI(SCR-01). WPM 비원어민 보정. **pause는 whisper word timestamp가 부정확(`-ml 1` gap 붕괴) → ffmpeg silencedetect로 오디오에서 직접 측정(STT 독립).** 활용 3(회귀 eval) **구현됨**: `lib/eval/script-quality.ts`(scoreScriptQuality: coverage·estimatedWpm 적합도) + `scripts/eval.ts`(`pnpm eval`, stub 결정적/live 실모델) + Reviewer 훅(docs/automation.md). 하드 게이트=커버리지(green 베이스라인), 페이싱은 품질 추이 정보. **eval이 실측한 현 품질 갭: 생성 스크립트가 목표 시간 대비 너무 짧음**(live 26~55wpm 분량 vs 150 목표) → 활용 2(자가개선 루프)에서 교정.

활용 2(자가개선 루프) **구현됨**: `lib/ai/refine.ts generateWithRefinement`(생성→scoreScriptQuality→분량 미달/초과면 lengthBias로 재생성→더 나은 쪽 채택) + 프롬프트 분량 가이드(목표 단어수 명시). demo 핸들러가 사용. 실측(hermes3:8b): 프롬프트로 분량 ↑(피칭 186→312단어), 루프가 더 긴 초안 채택(179→194). **잔여 갭은 8B 모델 용량 한계** → 어댑터로 더 큰 모델/클라우드 교체 시 수렴(인프라 정상). 향후: maxRetries↑·few-shot 예시·RAG는 더 큰 모델에서 효과.

**실측 검증(2026-06-21, qwen2.5:14b, M2 Pro 16GB)**: 가설 부분 확인.
- 생성 분량: 5분 피칭 62→105wpm(524단어), 데모 67→91wpm — 8b 대비 약 1.7배. 단 10분 강연은 14b도 부족(390단어) → 긴 발표 완전 수렴은 32B+/클라우드 필요. 인프라(프롬프트 분량 가이드+자가개선 루프)는 모델 무관하게 동작, 품질은 모델 크기 비례 → **가설 확정**.
- 번역: 8b의 미번역(churn)·깨진 숫자("십만억")가 14b에서 대부분 해소(자연스러운 번역). 잔여: 큰 수 단위 현지화(14 billion→ko/zh 10배 오류, ja 정확) — LLM 공통 약점, 더 큰 모델/후처리 필요.
- 조치: README에 모델 권장표(8b 기본 / **qwen2.5:14b 권장(16GB+)** / 32B+·클라우드) 추가. 기본값은 진입장벽 위해 8b 유지.
...
### 임계 튜닝 (2026-07-02) — 기본값 0.5 → **0.3** (N=200 대표본으로 정정)

gop.py 원시 confidence를 1회 수집해 오프라인 임계 스윕. **1차(N=50, 앞 50개)는 0.2를 시사했으나 gold 오발음 5개뿐이라 노이즈** → **2차(N=200, 시드셔플로 화자 편중 제거, gold 86개)로 재검증 → 최적 F1 임계 = 0.3**.

**N=200 스윕(대표본, 최종 근거):**

| threshold | precision | recall | F1 | Spearman |
|---|---|---|---|---|
| 0.2 | 0.206 | 0.351 | 0.260 | 0.505 |
| **0.3 (채택)** | **0.224** | **0.485** | **0.306** | 0.531 |
| 0.5 (기존) | 0.187 | 0.608 | 0.286 | 0.550 |

- **최적 F1 = 0.3** — 0.35↑에서 precision 붕괴(0.35: 0.174). recall은 0.3까지 손실 없이 상승(0.351→0.485).
- confidence 분포(N=200): **정상 p50 0.75 vs 오발음 p50 0.25 — 겹침 큼**. N=50이 시사한 "0.2·강한 분리"는 소표본 착시였고, 대표본에선 분리가 약해 단어 precision은 **~0.22가 이 방법의 구조적 상한**(잔여 FP=일부 정상 단어의 강제정렬 실패, 임계로 못 잡음).
- 발화 단조성 Spearman은 0.5 전후에서 최고(0.55)지만 F1(단어 검출)은 0.3이 최적 — **단어 플래그=0.3, 발화 점수 신뢰도는 별개로 양호**.

**결론**: `WAV2VEC2_GOP_THRESHOLD` 기본값 **0.3**. 절대 정확도(F1 0.31)는 여전히 낮음. 재현: `SPEECHOCEAN762_DIR=/path PRONUNCIATION_SCORER=wav2vec2 LIMIT=200 pnpm eval:pronunciation`.

### 잔여 FP 근본 원인 진단 (2026-07-02) + 개선 계획

정상 단어 conf≈0(FP)의 원인은 **정렬 드리프트가 아니라 다국어 모델의 음소 혼동**이다(진단으로 확인).

- 모델 `wav2vec2-lv-60-espeak-cv-ft`는 392개 **다국어** espeak 음소를 쓴다. 억양 강한 L2(예: 만다린 L1) 발화에서 자유디코드가 **만다린식 토큰**(`ɕ i5`, 성조 숫자 `5`)을 뱉어, 정답 영어 음소(`ʃ iː`)가 **NW·사후확률 양쪽에서 낮은 값**을 받는다(같은 소리의 타 언어 토큰에 확률을 뺏김).
- 실측 예: `SHE ASKED, LOOKING UP AT ME`(전문가 acc 7=양호) → 디코드 `ɕ i5 ɕ i5 ɑ5 s …`, she/up/at conf 0. 강제정렬 사후확률 GOP도 동일하게 0 → **정렬 견고화로는 못 고침.**
- 영향: 정상 단어의 ~13.8%가 FP(N=200), precision ~0.22의 구조적 상한 원인.

**개선 계획 A (런치 후, 스코어링 코어 변경이라 충분한 검증 필요)**:
1. **토큰 정규화 + 음소 클래스 합산 GOP** — 성조 숫자·길이표시(`ː`) 제거하고 혼동 음소(예: `ɕ`↔`ʃ`)를 한 클래스로 묶어, 참조 음소의 GOP를 **클래스 단위 사후확률 합**으로 계산("그 소리가 났나"를 견고하게 측정). 모음 혼동 대부분 해소 예상, 자음은 소규모 매핑.
2. 또는 **영어 전용 음소 모델로 교체**(언어 간 누수 원천 제거, 더 근본적·재검증 큼).

그 전까지: GOP는 **강한 억양에서 과검출**함을 인지하고 사용(실패 시 휴리스틱 자동 폴백은 유효). 이 진단이 재작업의 출발점.
