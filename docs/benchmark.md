...
### 임계 튜닝 (2026-07-02) — 기본값 0.5 → 0.2

같은 N=50 표본에서 gop.py 원시 confidence를 1회 수집해 오프라인 임계 스윕:

| threshold | precision | recall | F1 | fp | Spearman |
|---|---|---|---|---|---|
| 0.5 (기존) | 0.077 | 0.60 | 0.136 | 36 | 0.312 |
| **0.2 (신규 기본)** | **0.176** | 0.60 | **0.273** | 14 | 0.387 |

confidence 분포: **정상 단어 mean 0.74·p50 0.857 vs 오발음 mean 0.2·p50 0** (분리됨) → 문제는 순전히 임계였음(0.5가 정상 단어까지 다수 플래그). 스윕상 **0.05~0.25가 최적대**(동일 PRF), 0.3↑에서 precision 재하락 → **0.2 채택**(여유). 효과: **FP 36→14, precision·F1 2배↑, recall 손실 0(0.60 유지)**.

**한계**: gold 오발음 5개뿐(tp=3, fn=2)이라 절대 수치는 여전히 낮고 노이즈 큼 — **방향(임계 하향)은 명확하나 무작위 대표본 재검증 필요**. 일부 정상 단어가 conf≈0(강제정렬 실패 등)로 남는 잔여 FP는 임계로 못 잡음 → 다음 개선 대상(정렬 견고화).
