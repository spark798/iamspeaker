...
![iamspeaker 데모 — 슬라이드 업로드 → AI 시범 발표 → 전달력 코칭](docs/img/demo.gif)
...
> **알려진 한계(실측).** 음소 모델이 다국어(espeak)라, **억양이 강한 L2 발화에선 정상 단어를 과검출**할 수 있다 — 정답 영어 음소가 같은 소리의 타 언어 토큰에 확률을 뺏긴다(예: 영어 `ʃ iː`를 만다린식 `ɕ i5`로). 단어 단위 precision은 제한적이며([`docs/benchmark.md`](docs/benchmark.md) 참고), 발화 단위 점수가 더 신뢰할 만하다. 더 견고한 스코어러(음소 클래스 사후확률 / 영어 전용 모델)는 계획됨.
