...
## 기능
슬라이드 업로드(PPTX/PDF) → 슬라이드 분석 → AI 데모 발표(스크립트 + Piper TTS 음성) → 스크립트 편집 → 연습 녹음 → 분석 리포트(WPM·필러·발음·시간배분 + **장르·모국어 보정 기준선 대비 점수**, **PDF 내보내기**) → 개선 스크립트 제안(자가개선 루프) → 예상 Q&A 대비 → 다국어 번역·자막(SRT) → 회차별 추이 그래프. UI 5개 언어(ko/en/ja/zh/es).
